<?php 
	...
	class Nepali_Calendar {
        ...
        private function prepare_day_of_month($day)
        {
            $result='';
            $arr_chars = str_split($day);
            foreach($arr_chars as $arr_char)
            {
                $result.= $this->get_day_of_month($arr_char);
            }
            return $result;
        }
        private function get_day_of_month($day)
        {
		...
		public function eng_to_nep($yy,$mm,$dd){
			if ($this->is_range_eng($yy, $mm, $dd) == false){
                ...
                $this->nep_date["year"] = $this->prepare_day_of_month($y) ;
                $this->nep_date["month"] = $this->prepare_day_of_month($m) ;
                $this->nep_date["date"] = $this->prepare_day_of_month($total_nDays) ;
                $this->nep_date["num_day"] = $this->prepare_day_of_month($numDay) ;
